...
func (gq *GroupQuery) QueryActors() *ActorQuery {
	query := &ActorQuery{config: gq.config}
...
func (gq *GroupQuery) Clone() *GroupQuery {
	...
	return &GroupQuery{
		...
		withActors: gq.withActors.Clone(),
		withOwners: gq.withOwners.Clone(),
...
func (gq *GroupQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Group, error) {
	...
	if gq.withActors != nil || gq.withOwners != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := gq.withActors; query != nil {
		if err := gq.loadActors(ctx, query, nodes, nil,
			func(n *Group, e *Actor) { n.Edges.Actors = e }); err != nil {
			return nil, err
		}
	}
	if query := gq.withOwners; query != nil {
		if err := gq.loadOwners(ctx, query, nodes, nil,
			func(n *Group, e *Actor) { n.Edges.Owners = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (gq *GroupQuery) loadActors(ctx context.Context, query *ActorQuery, nodes []*Group, init func(*Group), assign func(*Group, *Actor)) error {
	ids := make([]uint64, 0, len(nodes))
	nodeids := make(map[uint64][]*Group)
	for i := range nodes {
		if nodes[i].group_actors == nil {
			continue
		}
		fk := *nodes[i].group_actors
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "group_actors" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (gq *GroupQuery) loadOwners(ctx context.Context, query *ActorQuery, nodes []*Group, init func(*Group), assign func(*Group, *Actor)) error {
	ids := make([]uint64, 0, len(nodes))
	nodeids := make(map[uint64][]*Group)
	for i := range nodes {
		if nodes[i].group_owners == nil {
			continue
		}
		fk := *nodes[i].group_owners
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "group_owners" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
