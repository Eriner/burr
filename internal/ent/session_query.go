...
func (sq *SessionQuery) prepareQuery(ctx context.Context) error {
	...
	if err := session.Policy.EvalQuery(ctx, sq); err != nil {
		return err
	}
	return nil
}
...
func (sq *SessionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Session, error) {
	...
	if query := sq.withAccounts; query != nil {
		if err := sq.loadAccounts(ctx, query, nodes, nil,
			func(n *Session, e *Actor) { n.Edges.Accounts = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (sq *SessionQuery) loadAccounts(ctx context.Context, query *ActorQuery, nodes []*Session, init func(*Session), assign func(*Session, *Actor)) error {
	ids := make([]uint64, 0, len(nodes))
	nodeids := make(map[uint64][]*Session)
	for i := range nodes {
		if nodes[i].session_accounts == nil {
			continue
		}
		fk := *nodes[i].session_accounts
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "session_accounts" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
